...
export default function App() {
  ...
  const [futuresCoins, setFuturesCoins] = useState<string[]>([]);
  const [coinQuery, setCoinQuery] = useState('');
  const selectorHydrated = useRef(false);
  ...
  const load = async () => {
    try {
      ...
      // Background polling must not overwrite an unsaved selector choice.
      if (!selectorHydrated.current || !selectorDirty) {
  ...
  useEffect(() => {
    let cancelled = false;
    const loadFuturesCoins = async () => {
      try {
        const r = await fetch('/api/v1/markets', { headers: authHeaders });
        const j = await r.json().catch(() => ({}));
        const coins = Array.isArray(j?.markets)
          ? j.markets.map((m:any) => String(m?.symbol || '').toUpperCase()).filter((s:string) => /^[A-Z0-9._-]+\/USDT$/.test(s))
          : [];
        if (!cancelled) setFuturesCoins(Array.from(new Set(coins)).sort());
      } catch {}
    };
    loadFuturesCoins();
    const id = setInterval(loadFuturesCoins, 60000);
        ...
        <section className="rounded-xl border border-blue-500/20 bg-[#10151e] p-4">
          ...
          {settingsOpen && <div className="mt-4 space-y-4">
            <div className="grid md:grid-cols-3 gap-3">
              <button onClick={()=>{setSelectorDirty(true); setSelectorMode('manual');}} className={`rounded-xl border p-4 text-left ${selectorMode==='manual'?'border-blue-500 bg-blue-500/10':'border-[#293142] bg-[#0b0f16]'}`}><b>Profesyonel</b><div className="text-xs text-slate-500 mt-1">Senin belirlediğin coin havuzunda mevcut profesyonel mikro-yapı algoritması en iyi adayı seçer.</div></button>
              <button onClick={()=>{setSelectorDirty(true); setSelectorMode('algorithmic');}} className={`rounded-xl border p-4 text-left ${selectorMode==='algorithmic'?'border-emerald-500 bg-emerald-500/10':'border-[#293142] bg-[#0b0f16]'}`}><b>Algoritma</b><div className="text-xs text-slate-500 mt-1">USDT Futures piyasasını tarar ve kriterlere uyan en güçlü coin'i seçer.</div></button>
              <button onClick={()=>{setSelectorDirty(true); setSelectorMode('ai');}} className={`rounded-xl border p-4 text-left ${selectorMode==='ai'?'border-violet-500 bg-violet-500/10':'border-[#293142] bg-[#0b0f16]'}`}><b>🤖 Yapay Zekâ</b><div className="text-xs text-slate-500 mt-1">Profesyonel sekmeyle aynı canlı kâr/zarar korumasını kullanır; ayrıca order-flow, whale, likidite, EV ve piyasa rejimini değerlendirip işlem, pozisyon ve kaldıraç kararını risk profili sınırlarında verir.</div></button>
            </div>
            <div className="rounded-xl border border-amber-500/20 bg-[#0b0f16] p-4">
              ...
              <div className="grid grid-cols-3 gap-2">
                {([['conservative','🟢 Muhafazakâr','En düşük risk · AI max 4X'],['balanced','🟡 Orta','Dengeli risk · AI max 8X'],['aggressive','🔴 Riskli','Yüksek risk · AI max 12X']] as const).map(([key,label,desc]) => <button key={key} onClick={()=>{setSelectorDirty(true); setRiskMode(key)}} className={`rounded-lg border p-3 text-left ${riskMode===key?'border-amber-400 bg-amber-400/10':'border-[#293142]'}`}><b>{label}</b><div className="text-[10px] text-slate-500 mt-1">{desc}</div></button>)}
              </div>
            </div>
            <div className="grid md:grid-cols-2 gap-3">
              <div className="rounded-lg bg-[#0b0f16] p-3 text-xs">
                 <div className="font-medium">Profesyonel coinler (en fazla 10)</div>
                 <div className="mt-2 flex flex-wrap gap-2">
                   {manualPairs.split(',').map(x=>x.trim().toUpperCase()).filter(Boolean).map(pair => (
                     <button type="button" key={pair} onClick={()=>{setSelectorDirty(true); setManualPairs(manualPairs.split(',').map(x=>x.trim().toUpperCase()).filter(x=>x && x!==pair).join(', '));}} className="rounded-full border border-emerald-500/50 bg-emerald-500/15 px-3 py-1.5 font-mono text-xs text-emerald-300 hover:bg-emerald-500/25">{pair} ×</button>
                   ))}
                 </div>
                 <input value={coinQuery} onChange={e=>setCoinQuery(e.target.value.toUpperCase())} className="mt-2 w-full rounded-md border border-[#293142] bg-[#080b11] p-2 font-mono text-xs outline-none" placeholder="Coin ara: BTC, ETH, SOL..." />
                 {coinQuery.trim() && (
                   <div className="mt-2 max-h-48 overflow-y-auto rounded-md border border-[#293142] bg-[#080b11]">
                     {futuresCoins.filter(c=>c.startsWith(coinQuery.trim())).slice(0,100).map(pair => {
                       const selected = manualPairs.split(',').map(x=>x.trim().toUpperCase()).includes(pair);
                       return <button type="button" key={pair} disabled={selected} onClick={()=>{if(selected)return; const current=manualPairs.split(',').map(x=>x.trim().toUpperCase()).filter(Boolean); if(current.length>=10)return; setSelectorDirty(true); setManualPairs([...current,pair].join(', ')); setCoinQuery('');}} className={`block w-full px-3 py-2 text-left font-mono ${selected?'text-slate-600 cursor-not-allowed':'text-slate-200 hover:bg-emerald-500/10 hover:text-emerald-300'}`}>{pair}</button>;
                     })}
                     {!futuresCoins.some(c=>c.startsWith(coinQuery.trim())) && <div className="px-3 py-2 text-slate-500">Bu harfle başlayan Binance Futures coin bulunamadı.</div>}
                   </div>
                 )}
                 <div className="mt-2 text-[10px] text-slate-500">Bir harf yazınca Binance USDT-M Futures listesindeki eşleşen coinler gösterilir. Coin'e tıklayınca yeşil etiket olarak eklenir.</div>
               </div>
              <label className="rounded-lg bg-[#0b0f16] p-3 text-xs">Maksimum açık pozisyon<input type="number" min="1" max="10" value={maxPositions} onChange={e=>{setSelectorDirty(true); setMaxPositions(Number(e.target.value))}} className="mt-2 w-full rounded-md border border-[#293142] bg-[#080b11] p-2 font-mono"/><span className="text-[10px] text-amber-400 block mt-1">Not: mevcut işlem yöneticisi şu sürümde tek aktif net Futures pozisyonunu yönetiyor; 2–10 kapasite ayarı seçim/risk limiti olarak kaydedilir, çoklu pozisyon yöneticisi ayrı refaktördür.</span></label>
              <label className="rounded-lg bg-[#0b0f16] p-3 text-xs">Algoritmik tarama coin sayısı<input type="number" min="5" max="50" value={scanAssets} onChange={e=>{setSelectorDirty(true); setScanAssets(Number(e.target.value))}} className="mt-2 w-full rounded-md border border-[#293142] bg-[#080b11] p-2 font-mono"/></label>
              <label className="rounded-lg bg-[#0b0f16] p-3 text-xs">Minimum fırsat skoru<input type="number" min="0.2" max="0.95" step="0.01" value={minOpportunity} onChange={e=>{setSelectorDirty(true); setMinOpportunity(Number(e.target.value))}} className="mt-2 w-full rounded-md border border-[#293142] bg-[#080b11] p-2 font-mono"/></label>
            </div>
