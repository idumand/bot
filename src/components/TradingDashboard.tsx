...
interface TradingDashboardProps {
  ...
  logs: LogEntry[];
  onForceEntry: (pair: string, side: 'long' | 'short') => void;
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-[#151921] border border-[#1e232f] p-4 rounded-xl shadow-lg relative overflow-hidden">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Toplam Kâr/Zarar</span>
            <div className={`p-2 rounded-lg ${metrics.total_pnl_usdt >= 0 ? 'bg-emerald-500/10 text-emerald-400' : 'bg-rose-500/10 text-rose-400'}`}>
              {metrics.total_pnl_usdt >= 0 ? <TrendingUp className="w-5 h-5" /> : <TrendingDown className="w-5 h-5" />}
            </div>
          </div>
          ...
          <div className="mt-2 text-xs text-slate-400">
            Günlük K/Z: <span className={metrics.daily_pnl_usdt >= 0 ? 'text-emerald-400 font-mono font-semibold' : 'text-rose-400 font-mono font-semibold'}>
              {metrics.daily_pnl_usdt >= 0 ? '+' : ''}${metrics.daily_pnl_usdt.toFixed(2)} USDT
            </span>
          </div>
        </div>

        {/* Win Rate & Trades Count */}
        <div className="bg-[#151921] border border-[#1e232f] p-4 rounded-xl shadow-lg">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Kazanma Oranı</span>
            <div className="p-2 rounded-lg bg-indigo-500/10 text-indigo-400">
              <Award className="w-5 h-5" />
            </div>
          </div>
          <div className="mt-2 flex items-baseline space-x-2">
            <span className="text-2xl font-mono font-bold text-white">{metrics.win_rate}%</span>
            <span className="text-xs text-slate-400">({metrics.winning_trades}W / {metrics.losing_trades}L)</span>
          </div>
          <div className="mt-2 text-xs text-slate-400">
            Gerçekleşen İşlem: <span className="text-slate-200 font-mono">{metrics.total_trades}</span>
          </div>
        </div>
        ...
        <div className="bg-[#151921] border border-[#1e232f] p-4 rounded-xl shadow-lg">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Açık Pozisyonlar</span>
            <div className="p-2 rounded-lg bg-amber-500/10 text-amber-400">
              <Zap className="w-5 h-5" />
            </div>
          </div>
          <div className="mt-2 flex items-baseline space-x-2">
            <span className="text-2xl font-mono font-bold text-white">{openTrades.length}</span>
            <span className="text-xs text-slate-400">/ {metrics.max_open_trades} Maks.</span>
          </div>
          <div className="mt-2 text-xs text-slate-400">
            İşlem Tutarı: <span className="text-slate-200 font-mono">${metrics.stake_amount} USDT</span>
          </div>
        </div>
        ...
        <div className="bg-[#151921] border border-[#1e232f] p-4 rounded-xl shadow-lg">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Risk Metrikleri</span>
            <div className="p-2 rounded-lg bg-emerald-500/10 text-emerald-400">
              <Activity className="w-5 h-5" />
            </div>
          </div>
          <div className="mt-2 flex items-baseline space-x-2">
            <span className="text-2xl font-mono font-bold text-white">{metrics.sharpe_ratio}</span>
            <span className="text-xs text-slate-400">Sharpe</span>
          </div>
          <div className="mt-2 text-xs text-slate-400">
            Maksimum Düşüş: <span className="text-rose-400 font-mono font-semibold">-{metrics.max_drawdown_pct}%</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-3 space-y-4">
          ...
          <div className="bg-[#151921] border border-[#1e232f] p-3 rounded-xl flex flex-wrap items-center justify-between gap-3">
            ...
            {/* Real Futures manual entry */}
            <div className="flex items-center gap-1.5">
              <button
                onClick={() => onForceEntry(selectedPair, 'long')}
                className="flex items-center space-x-1.5 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 border border-emerald-500/40 px-3 py-1.5 rounded-lg text-xs font-semibold transition"
              >
                <ArrowUpRight className="w-3.5 h-3.5" />
                <span>LONG</span>
              </button>
              <button
                onClick={() => onForceEntry(selectedPair, 'short')}
                className="flex items-center space-x-1.5 bg-rose-500/10 hover:bg-rose-500/20 text-rose-400 border border-rose-500/40 px-3 py-1.5 rounded-lg text-xs font-semibold transition"
              >
          ...
          <div className="flex-1 overflow-y-auto space-y-2 pr-1 max-h-[480px]">
            {markets.length === 0 ? (
              <div className="py-8 text-center text-slate-500 font-sans text-xs flex flex-col items-center justify-center space-y-2">
                ...
                <span>Binance Vadeli Piyasalar taranıyor...</span>
              </div>
            ) : (
              markets.map((m) => {
                const isSelected = m.symbol === selectedPair;
                const changePct = typeof m.change_24h_pct === 'number' ? m.change_24h_pct : 0;
                  ...
                  return (
                    <tr key={t.id} className="hover:bg-slate-800/30 transition">
                      ...
                      <td className="py-3 px-4 text-slate-400">
                        <div className="text-rose-400">SL: ${t.stop_loss_abs.toLocaleString()} ({t.stop_loss_pct.toFixed(2)}% ROI)</div>
                        {t.take_profit_pct && (
                          <>
                            <div className="text-emerald-400">TP: +{t.take_profit_pct.toFixed(2)}% ROI</div>
                            {t.adaptive_target_pct !== undefined && (
                              <div className="text-cyan-300">Hedef (1x): +{t.adaptive_target_pct.toFixed(1)}%{t.adaptive_target_reason ? ` · ${t.adaptive_target_reason}` : ''}</div>
                            )}
                          </>
                        )}
                      </td>
          ...
          {logs.slice(0, 5).map((log) => (
            <div key={log.id} className="flex items-center space-x-3 py-1 px-2 rounded bg-[#0b0e14] border border-slate-800/60">
              ...
              <span className={`px-1.5 py-0.2 rounded text-[10px] font-bold ${
                ...
                (log.level === 'WARN' || log.level === 'WARNING') ? 'bg-amber-500/20 text-amber-300' :
