...
export const OrderBookVisualizer: React.FC<OrderBookVisualizerProps> = () => {
  ...
  useEffect(() => {
    const fetchOrderBook = async () => {
      try {
        const res = await fetch('/api/v1/orderbook', { headers: authHeaders });
  ...
  const {
    ...
    deepScore = 0,
    confidence = 0,
    referenceTargetPct = 10,
    adaptiveTargetPct = 10,
    volatilityPct = 0,
    adaptiveTargetReason = '1x referans hedefi',
    probabilityLong = 0.5,
        ...
        <div className="space-y-4">
          ...
          <div className="bg-[#0b0e14] p-3 border border-emerald-500/20 rounded-lg grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <span className="text-lg font-bold font-mono text-emerald-300">+{Number(adaptiveTargetPct ?? referenceTargetPct ?? 10).toFixed(1)}%</span>
            </div>
            <div>
            ...
            <div className="col-span-2 text-[10px] text-slate-500 border-t border-[#2a3142] pt-2">
              {adaptiveTargetReason || '1x referans hedefi'} · Kaldıraç hedefi değiştirmez.
            </div>
          </div>
