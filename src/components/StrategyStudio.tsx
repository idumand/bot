...
export const StrategyStudio: React.FC<StrategyStudioProps> = ({
  ...
  const current = strategies[selectedStrategy] || strategies['SampleStrategy'];
  const [code, setCode] = useState(current.code_python);
  const [stoploss, setStoploss] = useState(current.stoploss);
  const [trailingStop, setTrailingStop] = useState(current.trailing_stop);
