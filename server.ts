...
async function startServer() {
  ...
  app.get('/api/v1/markets', async (_req, res) => {
    try {
      // The selector needs the COMPLETE Binance USDⓈ-M Futures symbol universe.
      // Do not return latestMarkets here: that telemetry list is intentionally
      // limited to the most liquid 100 markets for the scanner.
      const cachedAll = cachedFresh<any[]>('markets:all:futures', 60_000);
      if (cachedAll?.length) return res.json({ markets: cachedAll });
      ...
      const markets = Object.values(exchange?.markets || {})
        .filter((m: any) =>
          ...
          m?.linear === true &&
          m?.swap === true &&
          String(m?.quote || '').toUpperCase() === 'USDT'
        )
        .map((m: any) => {
          const symbol = `${String(m.base).toUpperCase()}/USDT`;
          return {
            symbol,
        ...
        .filter((m: any) => m.symbol && !m.symbol.includes('_'))
        .sort((a: any, b: any) => a.symbol.localeCompare(b.symbol));

      setCached('markets:all:futures', markets);
      res.json({ markets });
    } catch (e: any) {
      res.status(502).json({ markets: [], error: `Binance Futures coin listesi alınamadı: ${e.message}` });
    }
  });
