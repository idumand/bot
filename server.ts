...
type BinanceDiffState = { symbol:string; synced:boolean; syncing:boolean; lastUpdateId:number; buffer:any[]; bids:Map<string,number>; asks:Map<string,number>; gapCount:number; resyncCount:number; lastEventTs:number };
const binanceDiffStates = new Map<string, BinanceDiffState>();
const BINANCE_DIFF_DEPTH_LIMIT = 1000;
const BINANCE_DIFF_MAX_BUFFER = 5000;
function getBinanceDiffState(symbol:string): BinanceDiffState {
  let state=binanceDiffStates.get(symbol);
  if(!state){ state={symbol,synced:false,syncing:false,lastUpdateId:0,buffer:[],bids:new Map(),asks:new Map(),gapCount:0,resyncCount:0,lastEventTs:0}; binanceDiffStates.set(symbol,state); }
  return state;
}
function applyBinanceDiffEvent(state:BinanceDiffState,d:any){
  ...
  if(state.synced){ if(u<=state.lastUpdateId) return true; if(U>state.lastUpdateId+1){ state.synced=false; state.gapCount++; state.resyncCount++; state.buffer.length=0; return false; } }
...
async function syncBinanceDiffBook(symbol:string,generation:number){
  const state=getBinanceDiffState(symbol); if(state.syncing||generation!==eightStreamGeneration) return; state.syncing=true;
  try{
    const response=await fetch(`https://api.binance.com/api/v3/depth?symbol=${encodeURIComponent(symbol)}&limit=${BINANCE_DIFF_DEPTH_LIMIT}`);
    if(!response.ok) throw new Error(`snapshot ${response.status}`); const snapshot=await response.json(); if(generation!==eightStreamGeneration) return;
    ...
    if(!first){ state.synced=false; state.resyncCount++; return; }
  ...
  }catch(err:any){ state.synced=false; addEngineLog('WARN',`[8X BINANCE] diff-depth sync başarısız: ${String(err?.message||err)}`); }
...
function handleBinanceDiff(symbol:string,d:any,generation:number){
  ...
  if(!state.synced){ state.buffer.push(d); if(state.buffer.length>BINANCE_DIFF_MAX_BUFFER) state.buffer.splice(0,state.buffer.length-BINANCE_DIFF_MAX_BUFFER); void syncBinanceDiffBook(symbol,generation); return; }
  if(!applyBinanceDiffEvent(state,d)){ state.buffer.push(d); void syncBinanceDiffBook(symbol,generation); return; }
  const book=binanceDiffBook(state); if(book.bids.length&&book.asks.length) recordEightBookUpdate('binance',book);
}
...
function getBinanceFuturesDiffState(symbol:string): BinanceDiffState {
  ...
  if(!state){ state={symbol,synced:false,syncing:false,lastUpdateId:0,buffer:[],bids:new Map(),asks:new Map(),gapCount:0,resyncCount:0,lastEventTs:0}; binanceFuturesDiffStates.set(symbol,state); }
  return state;
}
...
async function syncBinanceFuturesDiffBook(symbol:string,generation:number){
  const state=getBinanceFuturesDiffState(symbol); if(state.syncing||generation!==streamGeneration) return; state.syncing=true;
  try{
    const response=await fetch(`https://fapi.binance.com/fapi/v1/depth?symbol=${encodeURIComponent(symbol)}&limit=1000`);
    if(!response.ok) throw new Error(`futures snapshot ${response.status}`);
    ...
    if(!applyBinanceDiffEvent(state,first)) { state.synced=false; state.resyncCount++; return; }
    for(const e of pending){ if(e===first) continue; if(!state.synced) break; if(!applyBinanceDiffEvent(state,e)) break; }
    const book=binanceFuturesDiffBook(state); if(book.bids.length&&book.asks.length) liveBooks.set(`futures:${symbol}`,book);
  }catch(err:any){ state.synced=false; addEngineLog('WARN',`[FUTURES NATIVE] diff-depth sync başarısız: ${String(err?.message||err)}`); }
  finally{ state.syncing=false; }
}
function handleBinanceFuturesDiff(symbol:string,d:any,generation:number){
  ...
  if(!state.synced){ state.buffer.push(d); if(state.buffer.length>BINANCE_DIFF_MAX_BUFFER) state.buffer.splice(0,state.buffer.length-BINANCE_DIFF_MAX_BUFFER); void syncBinanceFuturesDiffBook(symbol,generation); return; }
...
function applyConfig(conf: any) {
  ...
  professionalManualPairs = Array.from(new Set((Array.isArray(next.coin_selection?.professional_manual_pairs) ? next.coin_selection.professional_manual_pairs : []).filter((p:any) => typeof p === 'string' && p.includes('/')).map((p:string) => p.toUpperCase()))).slice(0, 10);
...
async function placeProtectiveStop(trade: TradeRecord): Promise<string | undefined> {
  ...
  try {
    ...
    addEngineLog('WARN', `[RİSK] Koruyucu stop borsa kaydı uyarısı: ${err?.message || err}`);
    return `fallback_stop_${Date.now()}`;
  }
}
...
async function openPosition(side: Side, symbol: string, requestedMargin?: number, adaptiveTargetPct?: number, adaptiveTargetReason?: string, requestedLeverage?: number) {
  ...
  try {
    ...
    trade.protective_order_id = `local_stop_${Date.now()}`;
    addEngineLog('WARN', `[GÜVENLİK] Stop yerel olarak izlenecek: ${stopErr?.message || stopErr}`);
  }
...
async function analyzeSimpleOrderBookPair(pair: string) {
  ...
  const accelerationLong = !simpleModeConfig.require_obi_acceleration;
  const accelerationShort = !simpleModeConfig.require_obi_acceleration;
  const wallLong = true;
...
async function analyzeFuturesPair(pair: string) {
  ...
  // No historical candles are retained or fetched for the decision engine.
  // Volatility is estimated from the live book itself (spread/depth pressure),
  // keeping the engine stateless with respect to market history.
  const volatilityPct = clamp(
    0.20 + SpreadPct * 100 * 8 + Math.abs(microBias) * 0.60 + Math.abs(depthPressure || 0) * 0.90,
    0.20, 6.0
  );
...
async function executeRealTradeLogic() {
  ...
  try {
    ...
    if (analysis.signal && analysis.scalpV2?.signal === analysis.signal && analysis.scalpV2.score >= SCALP_V2_MIN_SCORE) {
      ...
      activePosition.highConviction = Boolean(v27.highConviction);
      activePosition.tp1Done = false;
      activePosition.tp1Price = trade.tp1_price;
      activePosition.runnerTargetPrice = trade.runner_target_price;
      activePosition.runnerFraction = 1 - tp1Fraction;
      activePosition.optimizerTp1Fraction = tp1Fraction;
      activePosition.optimizerRunnerTrailPct = safeNum(optimizer.runnerTrailPct, V27_RUNNER_TRAIL_PCT);
      activePosition.optimizerRunnerTargetPct = target;
      activePosition.optimizerBucket = optimizer.bucket;
      activePosition.entryScore = analysis.scalpV2.score;
      activePosition.regimeQuality = v27.regimeQuality;
      activePosition.sizeMultiplier = v27.sizeMultiplier;
      if (coinSelectionMode === 'ai') {
        const aiPlanForLadder = await buildAiPositionPlan(analysis, analysis.signal);
        activePosition.ladderStep = 1;
        activePosition.ladderFractions = [0.40, 0.30, 0.30];
        activePosition.ladderTargetMargin = safeNum(aiPlanForLadder?.targetMargin, margin / 0.40);
        activePosition.ladderLastAddAt = 0;
        activePosition.ladderLastAddPrice = trade.open_rate;
      }
      saveTradingState();
    }
  } catch (error:any) {
...
(async () => {
  ...
  if (initialConf?.initial_state === 'running' || botState === 'running') {
    startTradingEngine();
  }
