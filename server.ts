...
function startMarketDataStreams(forceReconnect = false) {
  ...
  const futuresStreams = symbols.flatMap(s => [`${s}@depth20@100ms`, `${s}@aggTrade`]);
...
async function fetchBinancePublicKlines(pair: string, interval = '5m', limit = 80) {
  ...
  if (!rawKlines.length) {
    ...
    for (const url of endpoints) {
      try {
        const resp = await fetch(url, { headers: { 'User-Agent': 'sfeef-futures-engine/1.0' }, signal: AbortSignal.timeout(5000) });
        const text = await resp.text();
        if (resp.ok) {
          const json = JSON.parse(text);
          if (Array.isArray(json) && json.length) { rawKlines = json; break; }
        } else {
          let detail = text;
          try { detail = JSON.stringify(JSON.parse(text)); } catch {}
          addEngineLog('WARN', `Futures Kline HTTP ${resp.status} (${symbolClean} ${interval}): ${detail.slice(0, 240)}`);
        }
      } catch (e: any) {
        addEngineLog('WARN', `Futures Kline bağlantısı başarısız (${symbolClean} ${interval}): ${e?.message || e}`);
      }
    }
  }
